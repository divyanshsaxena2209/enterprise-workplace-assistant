...
let jobsPromise: Promise<any> | null = null;
...
export async function getJobs(page = 1, pageSize = 20, forceRefresh = false) {
  if (!forceRefresh && jobsCache) return jobsCache;
  if (!jobsPromise || forceRefresh) {
    jobsPromise = fetchWithAuth(`/jobs?page=${page}&page_size=${pageSize}`)
      .then(res => {
        jobsCache = res;
        return res;
      })
